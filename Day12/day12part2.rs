...
fn find_lowest_a(elevation_matrix: &matrix::Matrix<i32>, path_matrix: &matrix::Matrix<i32>) -> Position {
    let mut best: i32 = 10000;
    let mut position: Position = Position { x: 0, y: 0, elevation: 0, steps: 0 };

    for y in 0..elevation_matrix.height() {
        for x in 0..elevation_matrix.width() {
            if elevation_matrix.get(x, y) == 1 && path_matrix.get(x, y) > 0 && path_matrix.get(x, y) < best {
                position = Position { x: x, y: y, elevation: 0, steps: 0 };
                best = path_matrix.get(x, y);
            }
        }
    }
...
fn get_steps_to_end(elevation_matrix: &matrix::Matrix<i32>, path_matrix: &matrix::Matrix<i32>) -> i32 {
    let the_start = find_lowest_a(elevation_matrix, path_matrix);
